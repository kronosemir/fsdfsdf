...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'k!yayıddn') {
    msg.channel.sendMessage('@everyone **YAYdINDAYIZ BEYLER**!!!                                                                                                                                                                                                                                                                                                                                                         https://dlive.tv/KoDy');
  }
});
...
const youtube = new YouTube('AIzaSyC6wb9awCltTaljdxgL-7h6LIO6czIcfok');
const queue = new Map();
...
client.on("message", msg => {
  ...
  if (kufurEngel[msg.guild.id].küfürEngel === 'kapali') return;
    if (kufurEngel[msg.guild.id].küfürEngel=== 'acik') {
      const kufur = ["amk", "aq", "orospu", "oruspu", "oç", "sikerim", "yarrak", "piç", "amq", "sik", "amcık", "çocu", "sex", "seks", "amına", "orospu çocuğu", "sg", "siktir git", "a.q"];
  if (kufur.some(word => msg.content.toLowerCase().includes(word)) ) {
    if (!msg.member.hasPermission("ADMINISTRATOR")) {
      msg.delete()
       msg.reply("Küfür Silindi!").then(message => message.delete(3000));
    }
}
    }
});
